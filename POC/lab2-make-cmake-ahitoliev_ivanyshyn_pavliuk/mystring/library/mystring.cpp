...
void my_str_t::reserve(size_t new_capacity) {
    ...
    delete[] data_m;
    this->capacity_m = new_capacity + 8;
...
void my_str_t::insert(size_t idx, const my_str_t& str){
    ...
    if (size_m + other_size > capacity_m){
        reserve(size_m + other_size);
    }
    for (size_t i = size_m; i < idx; i--){
        data_m[i + other_size] = data_m[i];
    }
...
void my_str_t::insert(size_t idx, char c){
    ...
    size_m += 1;
    char insertion = data_m[idx];
    char temp;
    data_m[idx] = c;
    idx += 1;
    for (idx; idx < size_m; idx++){
        temp = data_m[idx];
        data_m[idx] = insertion;
        insertion = temp;
    }
}
//Bohdan
void my_str_t::insert(size_t idx, const char* cstr){
    ...
    if (cstr_size + size_m > capacity_m){
        reserve(cstr_size + size_m);
    }
    if (size_m + cstr_size > capacity_m){
        reserve(size_m + cstr_size);
    }
    for (size_t i = size_m; i < idx; i--){
        data_m[i + cstr_size] = data_m[i];
    }
...
size_t my_str_t::find(const std::string& str, size_t idx) {
    ...
    while (idx <= size_m - str_length) {
        size_t i = 0;
        while (i < str_length && data_m[idx + i] == str[i]) {
            ++i;
        }
        if (i == str_length) {
            return idx;
...
size_t my_str_t::find(const char* cstr, size_t idx) {
    ...
    while (idx <= size_m - cstr_length) {
        size_t i = 0;
        while (i < cstr_length && data_m[idx + i] == cstr[i]) {
            ++i;
        }
        if (i == cstr_length) {
            return idx;
...
my_str_t::~my_str_t() {
    delete[] data_m;
    data_m = nullptr; // Set the pointer to null to avoid double deletion
    size_m = 0;
    capacity_m = 0;
}
//Bohdan
bool operator==(const my_str_t& str1, const my_str_t& str2){
    ...
    const char* data_str1 = str1.c_str();
    const char* data_str2 = str2.c_str();
    size_t size_str = str1.size();
    for (size_t i = 0; i < size_str; i++){
        if (&data_str1[i] != &data_str2[i]) {
            return false;
        }
    }
    return true;
