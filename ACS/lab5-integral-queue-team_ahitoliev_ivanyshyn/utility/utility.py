...
def main():
    if len(sys.argv) != 5:
        print(f"Usage: {sys.argv[0]} <executable> <num_times> <threads_number> <points_per_task>")
        return 1

    executable = sys.argv[1]
    num_times = int(sys.argv[2])
    threads_number = int(sys.argv[3])
    points_per_task = int(sys.argv[4])
    epsilon = 10**-7
    ...
    with open('../data/output.csv', 'w', newline='') as csvfile:
        fieldnames = ['Function', 'Integral Value', 'Absolute Error', 'Relative Error', 'Minimal Time', \
                      'Average Time', 'Corrected Sample Standard Deviation', 'Threads', 'Points per Task']
        ...
        for j in range(1, 4):
            ...
            for i in range(1, num_times + 1):
                ...
                result = subprocess.run(["./" + executable, str(j), cfg_file, str(threads_number), str(points_per_task)], stdout=subprocess.PIPE)
            ...
            if check:
                ...
                writer.writerow({'Function': j, 'Integral Value': integral_value, 'Absolute Error': abs_error, \
                                 'Relative Error': rel_error, 'Minimal Time': min(times), 'Average Time': sum(times) / len(times), \
                                    ...
                                    'Points per Task': points_per_task})
            else:
