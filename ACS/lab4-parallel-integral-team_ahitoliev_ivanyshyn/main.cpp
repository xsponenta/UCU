...
int main(int argc, char* argv[]) {
    try {
        if (argc < 4) {
            std::cerr << "Usage: " << argv[0] << " <function_number> <cfg_file <threads_number>" << std::endl;
            return 1;
        }
        ...
        switch (function_number) {
            case 1:
                integrate_parallel(function_1, configMap, threads_number);
                break;
            case 2:
                integrate_parallel(function_2, configMap, threads_number);
                break;
            case 3:
                integrate_parallel(function_3, configMap, threads_number);
                break;
            default:
